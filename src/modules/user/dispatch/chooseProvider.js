...
const ChooseProvider = (props) => {
  ...
  const parcel_details = props.route.params.parcel_details;

  const ref = firestore()
    .collection("bids")
    .where("mongo_id", "==", parcel_details._id);
      ...
      {loading ? (
        ...
        >
          {bids.map((data) => {
            return (
              <>
                <View key={data.mongo_id}>
                  <Grid onPress={() => toggle_showModal("business_details")}>
